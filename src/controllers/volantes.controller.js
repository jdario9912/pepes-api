...
const { parsearValoresVolantes } = require('../services/parseo.valores');

const crearOrden = async (req, res) => {
  const { id_cliente, nro_orden, fecha_creacion, atendido_por, fecha_entrega, hora_entrega, muestra, tipo, tamano, cantidad, impresion, ubicacion_diseno, observaciones, total, entrega, estado, abono } = req.body;
  const datos = [parseInt(id_cliente), parseInt(nro_orden), fecha_creacion, atendido_por, fecha_entrega, hora_entrega, muestra, tipo, tamano, cantidad, impresion, ubicacion_diseno, observaciones, parseInt(total), parseInt(entrega), estado, abono];
...
const obtenerOrden = async (req, res) => {
  ...
  try {
    const [rows] = await pool.query(`SELECT * FROM volantes WHERE nro_orden = ${nro}`);
